...
// This function is run once per audio sample, so no heavy computations in here...
// e.g for the normal 8000Hz sample rate we run this 8000x per second, not too bad
...
void was_speaker() {
  LOG(DEBUG) << TAG("speak_detection") << AixLog::Color::YELLOW << "speaker was detected "
            << AixLog::Color::NONE << std::endl;
}
void was_no_speaker() {
  LOG(DEBUG) << TAG("speak_detection") << AixLog::Color::YELLOW << "no speaker was detected "
            << AixLog::Color::NONE << std::endl;
}
...
bool has_speaker() {
  ...
  double long_frame_average_energy = (double)long_energy / (double)long_frame_queue.size();
  ...
  speaker_is_detected = (frame_average_energy > long_frame_average_energy * 2);
